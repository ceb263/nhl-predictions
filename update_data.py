...
def main(start_date=None, end_date=None, season=2022, pbp_data_file=None, shifts_data_file=None, preseason_ratings_file='data/ratings_preseason.csv',
    output_playerGame=None, output_toiOverlap=None, output_teamGame='data/teamGame.pkl', replace=False):
    ...
    pbp = data_processing.add_xG_to_pbp(data_processing.get_shots_data(pbp, season=2022))
    ...
    playerGame.to_pickle(output_playerGame, index=False)
    del playerGame
    ...
    toi_overlap.to_pickle(output_toiOverlap, index=False)
    del toi_overlap

    if os.path.isfile(output_teamGame) and not replace:
        prev_teamGame = pd.read_pickle(output_teamGame)
    else:
        prev_teamGame = None
    teamGame = data_processing.aggregate_team_data(pbp, prev_teamGame)
    teamGame.to_pickle(output_teamGame, index=False)
...
if __name__ == '__main__':
    ...
    if args.dates is None:
        ...
        main(start_date = args.dates[0], end_date = args.dates[1], replace=args.replace)
